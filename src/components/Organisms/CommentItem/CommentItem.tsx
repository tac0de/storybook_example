    ...
    >
      ...
      {showReplyForm && (
        <div className={cx('reply-form')}>
          <TextInputForm onSubmit={handleReplySubmit} placeholder="답글을 입력하세요..." submitText="답글 작성" />
        </div>
      )}

      {/* 💬 답글 토글 버튼 (답글이 있을 때만 표시) */}
      {comment.replies > 0 && (
        <Button variant="text" size="sm" onClick={() => setShowReplies(!showReplies)} className={cx('reply-toggle')}>
